...
ENV LOCAL_MODEL_PATH="/app/models/qwen2.5-3b-instruct-q4_k_m.gguf"
ENV PORT=7860
EXPOSE 7860
...
# Download the 3B model from Hugging Face hub only if it is not present in the build context
RUN apt-get update && apt-get install -y wget && \
    if [ ! -f /app/models/qwen2.5-3b-instruct-q4_k_m.gguf ]; then \
        echo "Model not found in build context. Downloading 3B model from Hugging Face Hub..."; \
        wget -q -O /app/models/qwen2.5-3b-instruct-q4_k_m.gguf https://huggingface.co/Qwen/Qwen2.5-3B-Instruct-GGUF/resolve/main/qwen2.5-3b-instruct-q4_k_m.gguf; \
    else \
